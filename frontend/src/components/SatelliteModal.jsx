...
// Shared content used both in full-screen and inline modes
function InnerContent({ sat, badge, onClose, onAnalyze, analyzed, riskCount }) {
  return (
    <>
      {/* Header bar */}
      <div style={{
...
// When inline=true the component renders as scrollable content inside the
// existing sidebar — no fixed overlay, no backdrop, no animation.
export default function SatelliteModal({ sat, onClose, onAnalyze, analyzed = false, riskCount = 0, inline = false }) {
  // Close on Escape
  useEffect(() => {
    ...
    return (
      <>
        <div style={{
          ...
          <InnerContent
            sat={sat} badge={badge} onClose={onClose}
            onAnalyze={onAnalyze} analyzed={analyzed} riskCount={riskCount}
          />
        </div>
        <style>{`
